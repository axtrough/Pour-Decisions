package net.raccoon.will.pour_decisions.common.command;
                                                ...
                                                Commands.argument("amount", IntegerArgumentType.integer(1, 12))
                                                        .executes(context -> {
                                                            ...
                                                            if (gunStack != null) {
                                                                ...
                                                                gunStack.set(PDComponents.BULLET_CHAMBERS.get(), bullets);
                                                                gunStack.set(PDComponents.CURRENT_CHAMBER.get(), random.nextInt(chambers));
                                        ...
                                        .then(
                                                Commands.argument("amount", IntegerArgumentType.integer(1, 12))
                                                        .executes(context -> {
                                                            ...
                                                            if (gunStack != null) {
                                                                gunStack.set(PDComponents.NUM_CHAMBERS.get(), amount);

                                                                List<Integer> bullets = new ArrayList<>(gunStack.getOrDefault(PDComponents.BULLET_CHAMBERS.get(), new ArrayList<>()));
                                                                bullets.removeIf(b -> b >= amount);
                                                                gunStack.set(PDComponents.BULLET_CHAMBERS.get(), bullets);

                                                                int current = gunStack.getOrDefault(PDComponents.CURRENT_CHAMBER.get(), 0);
                                                                if (current >= amount) gunStack.set(PDComponents.CURRENT_CHAMBER.get(), 0);
